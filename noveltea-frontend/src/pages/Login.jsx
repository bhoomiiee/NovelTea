    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--color-tea-50)] px-4 relative overflow-hidden">
      ...
      >
        <div className="bg-[var(--color-tea-150,rgba(246,236,223,0.7))] p-8 text-center border-b border-[var(--color-tea-200)] relative">
          ...
          >
            <Coffee size={30} className="animate-pulse" />
          </motion.div>
          <h2 className="text-3xl font-extrabold text-[var(--color-tea-950)] font-serif tracking-wide">Welcome Back</h2>
          <p className="text-[var(--color-tea-700)] mt-2 font-medium text-sm">Brew a fresh, inspiring session.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-4 py-3.5 rounded-xl border border-[var(--color-tea-250,var(--color-tea-300))] focus:ring-2 focus:ring-[var(--color-tea-500)] focus:border-transparent outline-none transition-all bg-white/60 font-medium"
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            type="submit"
            className="w-full py-4 bg-[var(--color-tea-800)] hover:bg-[var(--color-tea-900)] text-white rounded-xl font-extrabold shadow-md hover:shadow-lg border-b-4 border-[var(--color-tea-950)] transition-all flex justify-center items-center gap-2 cursor-pointer text-sm tracking-wide"
          >
            <LogIn size={18} className="text-[var(--color-amber-gold)]" />
            Enter Study Lounge
          </motion.button>
 
          <p className="text-center text-sm text-[var(--color-tea-700)] font-medium">
            Don't have an account yet?{' '}
            <Link to="/register" className="font-extrabold text-[var(--color-tea-900)] hover:text-[var(--color-tea-950)] underline decoration-wavy decoration-[var(--color-tea-400)]">
              Register here
            </Link>
          </p>
        </form>
